...
export const Sidebar: React.FC = () => {
  ...
  return (
    <aside className="w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col h-screen shrink-0 transition-colors duration-300">
      {/* Brand Logo Header */}
      <div className="h-16 flex items-center gap-3 px-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/20">
        <div className="bg-gradient-to-tr from-blue-600 to-purple-600 p-1.5 rounded-lg">
          <BrandIcon className="w-5 h-5 text-white" />
        </div>
        <span className="font-bold text-sm bg-gradient-to-r from-blue-500 to-purple-500 dark:from-blue-400 dark:to-purple-400 bg-clip-text text-transparent tracking-wider">
          ANTIGRAVITY
        </span>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto p-4 space-y-1">
            ...
            className={({ isActive }) => 
              `flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-all group ${
                isActive 
                  ...
                  : 'text-slate-500 hover:text-slate-800 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-slate-800/50 border border-transparent'
              }`
            }
          >
        ...
        {isAdmin && (
          ...
          >
            <ShieldAlert className="w-4 h-4 group-hover:scale-110 transition-transform text-purple-600 dark:text-purple-500" />
      ...
      {/* Sidebar Footer */}
      <div className="p-4 border-t border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950/10 text-center">
        <span className="text-[10px] font-mono text-slate-500 dark:text-slate-650 tracking-wider">
